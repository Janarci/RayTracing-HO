...
void RTImage::setPixel(int x, int y, float r, float g, float b, int samplesPerPixel)
{
    ...
    int rInt = static_cast<uchar>(256 * rtweekend::clamp(r, 0.0, 0.999));
    int gInt = static_cast<uchar>(256 * rtweekend::clamp(g, 0.0, 0.999));
    int bInt = static_cast<uchar>(256 * rtweekend::clamp(b, 0.0, 0.999));
    ...
    //critical++;
    //std::cout << "critical value" << critical << std::endl;
